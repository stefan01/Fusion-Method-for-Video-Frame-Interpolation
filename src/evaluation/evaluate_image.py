...
def evaluate_image(args, prediction_im, target_im):
    """
    ...
    prediction = fusion_interp.crop_center(
        prediction_im.permute(1, 2, 0), args.dim, args.dim).permute(2, 0, 1)
    target = fusion_interp.crop_center(
        target_im.permute(1, 2, 0), args.dim, args.dim).permute(2, 0, 1)
