...
class Trainer:
    ...
    def predict(self, lab_frame1, lab_frame2, rgb_frame1, rgb_frame2, target, debug=False):
        ...
        # Adacof prediction
        with torch.no_grad():
            ada_frame1, ada_frame2, ada_pred, flow_var_map = self.adacof_model(rgb_frame1, rgb_frame2)
        ...
        rgb_pred = torch.cat([lab2rgb_single(l) for l in lab_pred], 0).to(self.device)
        ...
        h_freq = self.pyr.inv_filter(vals_high).detach().cpu().reshape(r_shape).mean(1)
        h_freq_ph = self.pyr.inv_filter(vals_high_ph).detach().cpu().reshape(r_shape).mean(1)
        h_freq_diff = torch.abs(h_freq - h_freq_ph)
        h_freq_diff = (h_freq_diff*100).clamp(min=0, max=1.0)
        h_freq_diff = torch.stack([torch.as_tensor(gaussian_filter(h, 5)) for h in h_freq_diff])
        ...
        freq_diff = self.pyr.inv_filter(vals_diff).detach().cpu().reshape(r_shape).mean(1)*30
        freq_diff_median = torch.stack([torch.as_tensor(median_filter(f, size=50)) for f in freq_diff])
        ...
        # Get baseline
        with torch.no_grad():
            _,_, ada_inbetween1, _ = self.adacof_model(rgb_frame1, phase_pred.reshape(r_shape).float())
            ada_inbetween1 = ada_inbetween1.to(self.device).float()
            
            _,_, ada_inbetween2, _ = self.adacof_model(phase_pred.reshape(r_shape).float(), rgb_frame2)
        ...
        # Debugging
        if debug:
            u_phase = torch.as_tensor(rgb_frame1.reshape(r_shape)[0]).detach().cpu()
            u_phase = transforms.ToPILImage()(u_phase)
            u_phase.save(f'./test_1.png')
            
            u_phase = torch.as_tensor(rgb_frame2.reshape(r_shape)[0]).detach().cpu()
            u_phase = transforms.ToPILImage()(u_phase)
            u_phase.save(f'./test_2.png')
            
            u_phase = torch.as_tensor(phase_pred.reshape(r_shape)[0]).detach().cpu()
            u_phase = transforms.ToPILImage()(u_phase)
        ...
        other = torch.cat([lab_frame1.reshape(r_shape), lab_frame2.reshape(r_shape)], 1).to(self.device).float()
        maps = torch.stack([ada_uncertainty, phase_uncertainty, flow_var_map], 1).to(self.device).float()
        save = self.args.save
    ...
    def train(self):
        ...
        for batch_idx, triple in enumerate(self.train_loader):
            ...
            prediction, phase_pred = self.predict(lab_frame1, lab_frame2, rgb_frame1, rgb_frame2, target)
            ...
            if batch_idx % 50 == 0:
                self.test(idx=int(batch_idx/50), paths=['counter_examples/lights/001.png', 'counter_examples/lights/002.png', 'counter_examples/lights/003.png'], name='lights')
    ...
    def test(self, idx=None, paths=None, name=''):
        ...
        with torch.no_grad():
            prediction, _ = self.predict(lab_frame1, lab_frame2, rgb_frame1, rgb_frame2, target, debug=True)
